...
class Ticket extends DataBase
{
    ...
    {
        $sql = "SELECT tickets.area, tickets.title, tickets.status, tickets.user_id, tickets.created_at, ticketdetails.message, ticketdetails.date_message, ticketdetails.response, ticketdetails.date_response FROM tickets INNER JOIN ticketdetails ON ticketdetails.ticket_id = tickets.id WHERE tickets.id = $id LIMIT 1";
        $query = $this->conexion->query($sql);
        return $query->fetch_all(MYSQLI_ASSOC);
    }
